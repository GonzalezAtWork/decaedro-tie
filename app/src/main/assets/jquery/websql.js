...
(function($) {
		...
		open: function(properties) {
			...
			if (window.openDatabase) {
				...
				if (!properties) {var properties = {}};
					// set properties
					properties.name 			= properties.name 			|| 'KALITERA';
					properties.version			= properties.verstion 		|| '1.0';
					properties.description 		= properties.description 	|| 'SisManut - Kalitera';
					properties.size				= properties.size 			|| 5*1024*1024;
		...
		query: function(query, properties) {
			...
			if (!db) {
				...
				// setup transactions for the queries
					...
					alert("Error processing SQL: "+ codigo + " - " + tx);
				}, function (){});
			}
			// chain me!
			return $;
	...
	var methods = {
		...
		**/
		load: function(properties) {
			...
			$.sql.query(query, {failure: properties.failure, success: function(rows) {
				if (rows) {
					if(properties.customWrap){
						for (var i = 0; i < rows.length; i++) {
							// create the custom wrapper
							var customWrap = properties.customWrap;
							for (var j in rows.item(i)) {
								var base = "***"+ j +"***";
								var novo = rows.item(i)[j];							
								customWrap = customWrap.split(base).join(novo);
							}
							// append custom
							obj.html(obj.html() + customWrap);
						}
					}else{
					...
					if (properties.callback) {
						properties.callback(obj);
						//$(obj).data('db').callback.call(obj);
					}
				} else {
					// but what if there are no results to display?
					if (properties.failure) {
						properties.failure(obj);
	...
	*    callback		= null			- function to be called after the sql execution
	*  returns jQuery
	*
	...
	**/
	$.fn.sql = function(method, propertiesDelta) {
		...
		if (propertiesDelta)  {
			...
			// from changes
			properties.customWrap   = propertiesDelta.customWrap 	|| properties.customWrap;
			...
			properties.callback		= propertiesDelta.callback 		|| properties.callback;
